...
from common.helper import normalize_content, get_first_non_whitespace_char
# from common import * 
...
class OSCAL_Content:
    ...
    def __init__(self, file_path_and_name, file_content, support=None, identifier="", support_db_path=None):
        ...
        self.original_content = normalize_content(file_content)
        ...
        logger.debug("Support Object creted")
    ...
    def __str__(self) -> str:
        out = ""
        out += f"file-name {self.file_name}\n"
        out += f"original-format: {self.original_format}\n"
        out += f"oscal-model: {self.oscal_model}\n"
        out += f"oscal-version: {self.oscal_version}\n"

        match self.original_format:
            case "xml":
                out += f"is-valid: {self.xml_is_valid}\n"
                out += f"validation-report: {self.xml_validation_report}\n"
                if self.json != "": out += f"json_transform-report: {self.json_transform_report}\n"
                if self.yaml != "": out += f"yaml_transform-report: {self.yaml_transform_report}\n"
            case "json":
                out += f"is-valid: {self.json_is_valid}\n"
                out += f"validation-report: {self.json_validation_report}\n"
                if self.xml != "": out += f"xml_transform-report: {self.xml_transform_report}\n"
                if self.yaml != "": out += f"yaml_transform-report: {self.yaml_transform_report}\n"
            case "yaml":
                out += f"is-valid: {self.yaml_is_valid}\n"
                out += f"validation-report: {self.yaml_validation_report}\n"
                if self.json != "": out += f"json_transform-report: {self.json_transform_report}\n"
                if self.xml != "": out += f"xml_transform-report: {self.xml_transform_report}\n"
        return out  
    ...
    def __format_verification(self): # Private method
        ...
        if not is_recognized_format:
            ...
            first_non_ws = get_first_non_whitespace_char(self.original_content)
            match first_non_ws:
                case "<":  #  OSCAL XML
        ...
        if is_recognized_format:
            match self.original_format:
                ...
                case _:
                    is_recognized_format = False
                    logger.error(f"Unable to proceed with validation. Invalid format requested: {is_recognized_format}")
    ...
    def validate(self, target_format=""):
        ...
        out_str = "- - - - - - " + target_format.upper() + " is " + helper.iif(self.is_valid, "valid", "INVALID") +  " (" + str(run_time.total_seconds()) + "s)"
